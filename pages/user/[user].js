  ...
  return (
    <div className={`page ${styles["user-profile"]}`}>
      <div className={`dflex ${styles["user-profile__top-card"]}`}>
        <UserCard aviWidth={200} aviHeight={200} round user={user} />

      <div className={styles["user-profile____details"]}>
        <div className={styles["user-profile__fullname"]}>
            {Filter.getFullName(user)}
          </div>
          <div className={`align-center ${styles["user-profile__location"]}`}>
            <ImLocation className="before-icon" />
            <div className="place">{user.location}</div>
          </div>

          <div className={styles["user-profile__bio"]}>{user.about}</div>
      ...
      <div className={`align-center ${styles["user-profile__awards"]}`}>
        {user.awards.map((award, i) => (
          <div key={i} className={styles.award}>
            <div className={styles["award-icon"]}>{award.icon}</div>
            <div className={styles["award-name"]}>{award.name}</div>
          </div>
        ))}
      </div>
